...
import { calculateCapitalWithOutputFloor, calculateBufferedCapitalCharge } from './pricing/capitalEngineCRR3';
import { calculateCrossBonusAdjustment } from './pricing/crossBonuses';
import { resolveDelegation, tierToLegacyApprovalLevel } from './pricing/delegationEngine';
...
export function resolveEffectiveTenors(deal: Transaction, models: BehaviouralModel[]): EffectiveTenors {
      ...
      if (model.type === 'NMD_Replication') {
        if (model.nmdMethod === 'Caterpillar' && model.replicationProfile?.length) {
          ...
          const totalWeight = model.replicationProfile.reduce(
            (s, t) => s + (Number.isFinite(t.weight) ? t.weight : 0),
            0
          );
          if (totalWeight > 0) {
...
export const calculatePricing = (
  ...
  try {
    ...
    if (deal.category === 'Asset' && deal.durationMonths < 12 && formulaSpec.lpFormula !== '50_50_DTM_1Y') {
      ...
      liquidityPremium = Math.max(
        liquidityPremium,
        PC.NSFR_FLOOR_WEIGHT * lp1Y + (1 - PC.NSFR_FLOOR_WEIGHT) * liquidityPremium
      );
    }
    ...
    const effectiveStability = deal.category === 'Liability' ? classifyDepositStability(deal) : undefined;
    ...
    const totalLiquidityCost = liquidityPremium + clcCharge + nsfrCharge + csrbbCharge + contingentLiquidityCharge;
    ...
    const allocatedCapitalPerUnit = deal.amount > 0 ? capitalCalc.totalCapitalRequired / deal.amount : 0;
    const capitalCharge = calculateBufferedCapitalCharge(capitalCalc, deal.amount, deal.targetROE, riskFreeRate);
    ...
    // See: utils/pricing/contexts/governance + docs/pivot/PIVOT_PLAN.md §Bloque E
