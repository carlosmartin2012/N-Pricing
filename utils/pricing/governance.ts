 ...
 * (VITE_GOVERNANCE_MODE = 'RAROC' | 'EVA'). Default EVA to the sane bands
 * but fall back to RAROC when the flag explicitly says so.
 */
...
export const computeEvaBp = (raroc: number, hurdleRate: number): number => {
  return Math.round((raroc - hurdleRate) * 100);
};
