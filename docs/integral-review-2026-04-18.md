...
|---|---|---|---|---|
| 1 | Tenancy hardening `config.ts` + `deals.ts` + `audit.ts` + integration tests | 3 d | **Crítico** (desbloquea STRICT=on) | Backend lead |
...
| 3 | Añadir `requireTenancy()` guard + test anti-regresión | 0.5 d | Alto | Backend lead |
...
| 7 | Split de `EntityOnboarding.tsx` en 3 sub-componentes + tests | 2 d | Medio | Frontend |
| 8 | Documentar `SSO_PROVIDER` explícito + default `PRICING_ALLOW_MOCKS=false` en `.env.example` | 0.25 d | Medio | DevOps |
