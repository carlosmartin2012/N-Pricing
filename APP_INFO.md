...
## Key Features
- **Pricing Engine**: Core FTP motor with 16 components (gaps) — base rate, liquidity premium, LCR/NSFR charges, ESG adjustments, capital charge, and full RAROC calculation.
...
- **Testing**: Vitest 4 (192 tests) + Playwright 1.59 (4 E2E specs) + Storybook 8.6
...
- `contexts/`: 5 React Context providers — Auth, Data, UI, Governance, MarketData.
- `hooks/`: Custom hooks including React Query wrappers (`queries/`) and decomposed Supabase sync (`supabaseSync/`).
- `components/`: 122 component files across 13 domain directories.
...
- `types.ts`: 52+ TypeScript interfaces for the entire domain model.
