...
export const PresenceAvatars: React.FC<PresenceAvatarsProps> = ({ users, maxVisible = 5 }) => {
  ...
  return (
    <div className="flex items-center -space-x-2">
      ...
      {overflow > 0 && (
        <div className="flex h-7 w-7 items-center justify-center rounded-full border-2 border-[var(--nfq-bg-root)] bg-[var(--nfq-bg-elevated)]/60 text-[10px] font-mono text-[color:var(--nfq-text-muted)]">
          +{overflow}
        </div>
      )}
    </div>
