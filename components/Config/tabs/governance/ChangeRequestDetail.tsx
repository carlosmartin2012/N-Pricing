      ...
      <div className="bg-[var(--nfq-bg-elevated)] border border-slate-700 rounded-lg p-4">
        <div className="flex items-start justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-xs text-[color:var(--nfq-text-muted)] mt-1">
              Submitters cannot self-approve. Admin and Risk Manager roles can review and apply queued changes.
            </p>
          </div>
          <div className="text-right">
            <div className="text-[10px] tracking-normal text-[color:var(--nfq-text-faint)]">Active Ruleset</div>
            <div className="text-sm font-semibold text-[color:var(--nfq-accent)]">
              {currentVersion?.label || 'Live governance baseline'}
            </div>
            <div className="text-[10px] text-[color:var(--nfq-text-faint)] font-mono">
              ...
              <div key={version.id} className="rounded border border-slate-800 bg-[var(--nfq-bg-root)]/60 p-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-[10px] font-mono text-[color:var(--nfq-text-faint)]">{version.id}</div>
                  </div>
                  <div className="text-right text-[10px] text-[color:var(--nfq-text-faint)]">
                    <div>{version.ruleCount} active rules</div>
                    <div>{new Date(version.createdAt).toLocaleString('en-GB')}</div>
                  </div>
                </div>
                <div className="mt-2 text-xs text-[color:var(--nfq-text-muted)]">{version.summary.reason}</div>
              </div>
            ))}
          </div>
        </div>
              ...
              <div key={task.id} className="rounded border border-slate-800 bg-[var(--nfq-bg-root)]/60 p-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="text-[10px] tracking-normal text-[color:var(--nfq-text-faint)]">
                      {task.scope} • {task.requiredRole}
                    </div>
                  </div>
                  <div className="text-[10px] text-[color:var(--nfq-warning)]">{task.status}</div>
                </div>
                <div className="mt-2 text-xs text-[color:var(--nfq-text-muted)]">{task.description}</div>
