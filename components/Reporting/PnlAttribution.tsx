    ...
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4 md:grid-cols-6">
        {[
          ...
          <div key={kpi.label} className="rounded-[var(--nfq-radius-card)] border border-[color:var(--nfq-border-ghost)] bg-[var(--nfq-bg-elevated)]/40 p-4">
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {attribution.map((item) => (
              ...
              >
                <td className="border-b border-[color:var(--nfq-border-ghost)] px-4 py-2 font-mono text-[var(--nfq-accent)] [font-variant-numeric:tabular-nums]">
                  {item.id}
                </td>
                <td className="border-b border-[color:var(--nfq-border-ghost)] px-4 py-2 text-[color:var(--nfq-text-tertiary)]">
                  {item.buName}
                </td>
                <td className="border-b border-[color:var(--nfq-border-ghost)] px-4 py-2 text-right font-mono text-[var(--nfq-success)] [font-variant-numeric:tabular-nums]">
                  {fmtM(item.nii)}
                </td>
                <td className="border-b border-[color:var(--nfq-border-ghost)] px-4 py-2 text-right font-mono text-[var(--nfq-warning)] [font-variant-numeric:tabular-nums]">
                  {fmtM(item.ftpCost)}
                </td>
                <td
                  className={`border-b border-[color:var(--nfq-border-ghost)] px-4 py-2 text-right font-mono font-bold [font-variant-numeric:tabular-nums] ${item.netMargin >= 0 ? 'text-[var(--nfq-accent)]' : 'text-[var(--nfq-danger)]'}`}
                >
                  {fmtM(item.netMargin)}
                </td>
                <td
                  className={`border-b border-[color:var(--nfq-border-ghost)] px-4 py-2 text-right font-mono [font-variant-numeric:tabular-nums] ${item.raroc >= 15 ? 'text-[var(--nfq-success)]' : item.raroc > 0 ? 'text-[var(--nfq-warning)]' : 'text-[var(--nfq-danger)]'}`}
                >
                  {item.raroc.toFixed(1)}%
                </td>
                <td className="border-b border-[color:var(--nfq-border-ghost)] px-4 py-2 text-right font-mono text-[10px] font-bold uppercase">
                  {item.approvalLevel}
                </td>
              </tr>
            ))}
            {attribution.length === 0 && (
              <tr>
                <td colSpan={7} className="py-8 text-center text-[color:var(--nfq-text-muted)]">
                  No booked deals
                </td>
              </tr>
            )}
