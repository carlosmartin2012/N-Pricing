  ...
  return (
    <Panel title="Transaction Deal Blotter" className="h-full">
      <div className="flex flex-col h-full">
        {/* Toolbar */}
        <div className="p-4 border-b border-slate-700 bg-slate-900 flex flex-wrap gap-4 justify-between items-center">
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative group">
              ...
              <input 
                ...
                className="bg-slate-950 border border-slate-700 rounded pl-9 pr-3 py-1.5 text-xs text-white focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/20 w-64 transition-all"
              />
            </div>
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-cyan-950/40 text-cyan-400 border border-cyan-900 rounded text-xs hover:bg-cyan-900/60 transition-colors"
            >
              <Upload size={12} /> Import
            </button>
            <button className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 text-slate-300 rounded border border-slate-700 text-xs hover:bg-slate-700 transition-colors">
        ...
        <div className="flex-1 overflow-auto bg-slate-900 relative">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-950 sticky top-0 z-10 shadow-sm">
              <tr>
                <th className="p-3 pl-4 text-[10px] uppercase font-bold text-slate-500 border-b border-r border-slate-800 w-32">Deal ID</th>
                <th className="p-3 text-[10px] uppercase font-bold text-slate-500 border-b border-r border-slate-800">Client ID</th>
                <th className="p-3 text-[10px] uppercase font-bold text-slate-500 border-b border-r border-slate-800 w-24">Product</th>
                <th className="p-3 text-[10px] uppercase font-bold text-slate-500 border-b border-r border-slate-800 w-24 text-right">Amount</th>
                <th className="p-3 text-[10px] uppercase font-bold text-slate-500 border-b border-r border-slate-800 w-20 text-right">Target Rate</th>
                <th className="p-3 text-[10px] uppercase font-bold text-slate-500 border-b border-r border-slate-800 w-28">Start Date</th>
                <th className="p-3 text-[10px] uppercase font-bold text-slate-500 border-b border-slate-800 w-28 text-center">Status</th>
                <th className="p-3 text-[10px] uppercase font-bold text-slate-500 border-b border-slate-800 w-20 text-center">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/50 text-xs text-slate-300">
              {filteredDeals.map((deal) => (
                <tr key={deal.id} className="hover:bg-slate-800/40 group transition-colors">
                  <td className="p-3 pl-4 border-r border-slate-800/50 font-mono text-cyan-500/80 group-hover:text-cyan-400">
                    {deal.id}
                  </td>
                  <td className="p-3 border-r border-slate-800/50 font-medium text-slate-200">
                    {deal.clientId}
                  </td>
                  ...
                  <td className="p-3 border-r border-slate-800/50 text-right font-mono text-slate-200">
                    {fmtCurrency(deal.amount, deal.currency)}
                  </td>
                  <td className="p-3 border-r border-slate-800/50 text-right font-mono font-bold text-slate-300">
                    {(3.0 + deal.marginTarget).toFixed(2)}%
                  </td>
                  <td className="p-3 border-r border-slate-800/50 text-slate-400">
                    {deal.startDate}
                  </td>
                  <td className="p-3 border-r border-slate-800/50 text-center">
                    <Badge variant={
                   ...
                   <td className="p-3 text-center flex justify-center gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEdit(deal)} className="text-slate-400 hover:text-cyan-400"><Edit size={14} /></button>
                    <button onClick={() => handleDelete(deal)} className="text-slate-400 hover:text-red-400"><Trash2 size={14} /></button>
                  </td>
                </tr>
              ))}
