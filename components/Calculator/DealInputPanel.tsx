...
const DealInputPanel: React.FC<Props> = ({ values, onChange, setDealParams, deals, clients, setClients, products, businessUnits }) => {
  const [showConfig, setShowConfig] = useState(false); // Default to clean view
  ...
  return (
    <Panel title="Pricing Simulation Engine" className="h-full">
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-8 relative">
            ...
            <div>
               <div className="flex justify-between mb-2">
                  ...
                  <span className="text-sm font-mono text-white font-bold">{values.durationMonths}m</span>
               </div>
               <input 
                  type="range" 
